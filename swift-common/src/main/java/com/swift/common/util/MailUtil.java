...
public class MailUtil {
	...
	static{
		host = PropertiesUtil.getString("system.mail.host");
		port = PropertiesUtil.getString("system.mail.port");
		username = PropertiesUtil.getString("system.mail.username");
		password = PropertiesUtil.getString("system.mail.password");
		sendFrom = PropertiesUtil.getString("system.mail.sendFrom");
		sendNick = PropertiesUtil.getString("system.mail.sendNick");
	}
	
	/**
	<!-- spring mail sender -->
	<bean id="javaMailSender" class="org.springframework.mail.javamail.JavaMailSenderImpl"  scope="singleton" >
		<property name="host" value="${system.mail.host}" />			<!-- SMTP发送邮件的服务器的IP和端口 -->
		<property name="port" value="${system.mail.port}" />
		<property name="username" value="${system.mail.username}" />	<!-- 登陆SMTP邮件发送服务器的用户名和密码 -->
		<property name="password" value="${system.mail.password}" />
			...
			exec.execute(new Thread(new Runnable() {
				...
				public void run() {
					while(total < 10){
						...
						sendMail("xxxx@163.com", "测试邮件", mailBody, false, null);
